...
#initialise qubits or device     
    def init_qubits(self, qubittype, n):
        ...
        if qubittype == 'NamedQubit':
            assert (all(isinstance(x, str) for x in n)),\
                "Error in qubit initialisation: n needs to be string list for NameQubit, received: n = {}, {}".format(n, type(n))
            ...
            assert (isinstance(n, (int, np.int_)) and n > 0),\
                "Error in qubit initialisation: n needs to be natural Number for LineQubit, received: n = {}, {}".format(n, type(n))
            #need this awkward return scheme to get right format
            #try:
            ...
            assert (np.size(n) == 2 and isinstance(n[0], (int, np.int_)) and n[0] > 0 \
                and isinstance(n[1], (int, np.int_))  and n[1] > 0),\
            ...
            assert False, "Invalid qubittype, received: '{}', allowed is \n \
                'NamedQubit', 'LineQubit', 'GridQubit'".format(qubittype)
        '''    
