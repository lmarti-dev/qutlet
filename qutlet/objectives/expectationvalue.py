...
class ExpectationValue(AbstractExpectationValue):
    ...
    def evaluate(self, wavefunction: np.ndarray, options: dict = {}) -> np.float64:
        assert len(self.__energy_fields) == len(
            self.__energies
        ), "Length of Pauli types and energy masks do not match, {}!={}".format(
            len(self.__energy_fields), len(self.__energies)
        )
        i = 0
        res = 0
    ...
    def __eq__(self, other):
        ...
        for key in self.__dict__.keys():
            ...
            if key == "_ExpectationValue__energies":
                temp_bools.append(
                    (getattr(self, key)[0] == getattr(other, key)[0]).all()
                )
                temp_bools.append(
                    (getattr(self, key)[1] == getattr(other, key)[1]).all()
                )
                continue
