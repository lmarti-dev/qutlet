...
class CVaR(Objective):
    ...
    def __init__(
        self, model: AbstractModel, alpha: Real, field: Literal["X", "Z"] = "Z"
    ):
        super().__init__(model)

        assert (
            0.0 <= alpha <= 1.0
        ), "cVaR alpha must be in (0, 1). Received: {:f}".format(alpha)
        assert field in [
            "Z",
            "X",
        ], "Bad input 'field'. Allowed values are ['X', 'Z' (default)], received {}".format(
            field
        )
    ...
    def _evaluate_x(self, wavefunction_z: np.ndarray) -> Real:
        ...
        return (
            CVaR._calc_cvar(_probabilities, _energies, self.__alpha) / self.__n_qubits
        )
    ...
    @staticmethod
    def _calc_cvar(
        probabilities: np.ndarray, energies: np.ndarray, alpha: Real
    ) -> Real:
