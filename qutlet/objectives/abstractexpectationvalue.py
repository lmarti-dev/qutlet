...
class AbstractExpectationValue(Objective):
    ...
    def __init__(
        self, model: AbstractModel, observable: Optional[cirq.PauliSum] = None
    ):
    ...
    def evaluate(
        ...
        if q_map is None:
            q_map = {
                self._model.qubits[k][l]: int(k * self._model.n[1] + l)
                for l in range(self._model.n[1])
                for k in range(self._model.n[0])
            }
        if np.size(wavefunction) == self._N:
            return np.real(
                self._observable.expectation_from_state_vector(
                    wavefunction, q_map, atol=atol
                )
            )
        elif np.size(wavefunction) == self._N**2:
            return np.real(
                self._observable.expectation_from_density_matrix(
                    wavefunction, q_map, atol=atol
                )
            )
        else:
