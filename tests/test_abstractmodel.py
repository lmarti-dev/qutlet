...
def test_diagonalise(qubittype, n, coefficients, gates, qubits, val_exp, vec_exp):
    ...
    for i in range(2):
        ...
        cirq.testing.lin_alg_utils.assert_allclose_up_to_global_phase(
            np_sol.eig_vec[:, i], scipy_sol.eig_vec[:, i], rtol=1e-15, atol=1e-15
        )
        cirq.testing.lin_alg_utils.assert_allclose_up_to_global_phase(
            vec_exp[:, i], scipy_sol.eig_vec[:, i], rtol=1e-15, atol=1e-15
        )
    ...
    [
        (
            cirq.PauliSum.from_pauli_strings(
                [
                    -np.pi * cirq.Z(cirq.LineQubit(i)) * cirq.Z(cirq.LineQubit(i + 1))
                    for i in range(8)
                ]
            )
            + cirq.PauliSum.from_pauli_strings(
...
def test_diagonalise_dense_sparse_speedup(paulisum):
    ...
    t2 = default_timer()
    print(
        "Numpy dense: {}\tScipy Sparse: {}\tSpeed-up: {}".format(
    ...
    # Test whether found eigenvalues are all close up to tolerance
    np.testing.assert_allclose(
        np_sol.eig_val[0:2], sparse_scipy_sol.eig_val, rtol=1e-13, atol=1e-14
    )
