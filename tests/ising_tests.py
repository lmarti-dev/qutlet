...
class IsingTester:
    ...
    def simple_energy_JZZ_hX_test(self, qubittype, n, j_v, j_h, h, test_gate, E_exp, basis):
        ...
        assert(abs(E_exp - wf_energy) < self.atol), \
            "Simple JZZ hX energy test failed; expected: {}, received {}, tolerance {}"\
            .format(E_exp, wf_energy, self.atol)
    ...
    def simple_energy_JZZ_hZ_test(self, qubittype, n, j_v, j_h, h, test_gate, E_exp):
        ...
        assert(abs(E_exp - wf_energy) < self.atol), \
            "Simple JZZ hZ energy test failed; expected: {}, received {}, tolerance {}"\
            .format(E_exp, wf_energy, self.atol)
...
#add the option to apply not to all qubits
    def simple_spin_value_map_test(self, qubittype, n, j_v, j_h, h, test_gate, vm_exp, app_to = []):
        ...
        assert(dict == type(vm_exp)), \
            "Ising, value map test failed: vm_exp expected to be dictionary, received: {}".format(type(vm_exp))
        ...
        assert(len(value_map) == len(vm_exp)), \
            "Ising, value map test failed: length expected: {}, received: {}".format(len(vm_exp), len(value_map))
        #If elements in value_map and vm_exp do not match receive KeyError and test fails
        for element in value_map:
            assert(np.allclose(value_map[element], vm_exp[element], rtol=0, atol=self.atol)),\
                "Ising, value map test failed: for element {}, expected: {}, received: {}".\
                    format(element, vm_exp[element], value_map[element])