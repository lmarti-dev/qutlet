...
import fauvqe.utilities.testing as testing_utils
...
def test_fswap_max(x_dimension, y_dimension, Z_snake, correct, horizontal, even):
    ...
    correct_gates = [
        of.FSWAP.on(
            fermi_hubbard.flattened_qubits[q[0]], fermi_hubbard.flattened_qubits[q[1]]
        )
        for q in correct
    ]
    ...
    print(gates)
    print(correct_gates)
    assert testing_utils.do_lists_have_same_elements(gates, correct_gates)
