...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,1))/5, \
            np.zeros((1,2))/10, cirq.X, -0.1, 'Z'),
        #Normalised energy; hence expect -j_v/2 = -0.1:
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,1))/5, \
            np.zeros((1,2))/10, cirq.X**2, -0.1,  'Z'),
        #Normalised energy; hence expect -j_h/2 = -0.25
        ('GridQubit', [2, 1], np.ones((1,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X, -0.25,  'Z'),
        #Normalised energy; hence expect -j_h/2 = -0.25
        ('GridQubit', [2, 1], np.ones((1,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X**2, -0.25, 'Z'),
        #Normalised energy; hence expect - 2h/2= -h:
        ('GridQubit', [1, 2], np.zeros((0,2))/2, np.zeros((1,1))/5, \
            np.ones((1,2))/3, cirq.X**2, -1/3,  'X'),
        #Normalised energy; hence expect + 2h/2 = h
        ('GridQubit', [1, 2], np.zeros((0,2))/2, np.zeros((1,1))/5, \
            np.ones((1,2))/3, cirq.Z, 1/3,  'X'),
        #############################################################
        #                   4 qubit tests                           #
        #############################################################
        #Normalised energy; hence expect -h
        ('GridQubit', [2, 2], np.zeros((1,2))/2, np.zeros((2,1))/5, \
            np.ones((2,2))/3, cirq.X, -1/3,  'X'),
        #Normalised energy; hence expect -h
        ('GridQubit', [2, 2], np.zeros((1,2))/2, np.zeros((2,1))/5, \
            np.ones((2,2))/3, cirq.X**2, -1/3,  'X'),
        #Normalised energy; hence expect h
        ('GridQubit', [2, 2], np.zeros((1,2))/2, np.zeros((2,1))/5, \
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.zeros((1,2))/10, cirq.X, -0.2, 'Z'),
        #Normalised energy; hence expect -j_v = -0.2:
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.zeros((1,2))/10, cirq.X**2, -0.2,  'Z'),
        #Normalised energy; hence expect -j_h = -0.5
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X, -0.5,  'Z'),
        #Normalised energy; hence expect -j_h = -0.5
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X**2, -0.5, 'Z'),
        #Normalised energy; hence expect - 2h/2= -h:
        ('GridQubit', [1, 2], np.zeros((1,2))/2, np.zeros((1,2))/5, \
            np.ones((1,2))/3, cirq.X**2, -1/3,  'X'),
        #Normalised energy; hence expect + 2h/2 = h
        ('GridQubit', [1, 2], np.zeros((1,2))/2, np.zeros((1,2))/5, \
            np.ones((1,2))/3, cirq.Z, 1/3,  'X'),
        ...
        #############################################################
        #Normalised energy; hence expect -h
        ('GridQubit', [2, 2], np.zeros((2,2))/2, np.zeros((2,2))/5, \
            np.ones((2,2))/3, cirq.X, -1/3,  'X'),
        #Normalised energy; hence expect -h
        ('GridQubit', [2, 2], np.zeros((2,2))/2, np.zeros((2,2))/5, \
            np.ones((2,2))/3, cirq.X**2, -1/3,  'X'),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        #Normalised energy; hence expect -j_v = -0.2:
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.zeros((1,2))/10, cirq.X, -0.2),
        #Normalised energy; hence expect -j_v = -0.2:
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.zeros((1,2))/10, cirq.X**2, -0.2),
        #Normalised energy; hence expect -j_h = -0.5
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X, -0.5),
        #Normalised energy; hence expect -j_h = -0.5
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.zeros((2,1))/10, cirq.X**2, -0.5),
        #Normalised energy; hence expect -j_v - h= -0.3:
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.ones((1,2))/10, cirq.X, -0.3),
        #Normalised energy; hence expect -j_v + h = -0.1
        ('GridQubit', [1, 2], np.ones((0,2))/2, np.ones((1,2))/5, \
            np.ones((1,2))/10, cirq.X**2, -0.1),
        #Normalised energy; hence expect -j_h-h = -0.6
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.ones((2,1))/10, cirq.X, -0.6),
        #Normalised energy; hence expect (-j_h+h) = -0.4
        ('GridQubit', [2, 1], np.ones((2,1))/2, np.ones((2,0))/5, \
            np.ones((2,1))/10, cirq.X**2, -0.4),
        #############################################################
        #                   4 qubit tests                           #
        #############################################################
        #Normalised energy; hence expect (-2j_h-2j_v-4h)/4 = -0.45
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((2,2))/5, \
            np.ones((2,2))/10, cirq.X, -0.8),
        #Normalised energy; hence expect (-2j_h-2j_v+4h)/4 = -0.6
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((2,2))/5, \
            np.ones((2,2))/10, cirq.X**2, -0.6),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ('GridQubit', [2, 1], np.zeros((1,1)), np.zeros((2,0)), 3.14*np.ones((2,1)), -3.14),
        ('GridQubit', [10, 1], np.zeros((9,1)), np.zeros((10,0)), 3.14*np.ones((10,1)), -3.14),
        ('GridQubit', [3, 1], np.zeros((2,1)), np.zeros((3,0)), 3.14*np.ones((3,1)), -3.14),
        ('GridQubit', [11, 1], np.zeros((10,1)), np.zeros((11,0)), 3.14*np.ones((11,1)), -3.14),
        ('GridQubit', [1, 2], np.zeros((0,2)), np.zeros((1,1)), 3.14*np.ones((1,2)), -3.14),
        ('GridQubit', [1, 10], np.zeros((0,10)), np.zeros((1,9)), 3.14*np.ones((1,10)), -3.14),
        ('GridQubit', [1, 3], np.zeros((0,3)), np.zeros((1,2)), 3.14*np.ones((1,3)), -3.14),
        ('GridQubit', [1, 11], np.zeros((0,11)), np.zeros((1,10)), 3.14*np.ones((1,11)), -3.14),
        ('GridQubit', [2, 1], 3.14*np.ones((2,1)), 3.14*np.ones((2,0)),   np.zeros((2,1)), -3.14),
        ('GridQubit', [10, 1], 3.14*np.ones((10,1)), 3.14*np.ones((10,0)), np.zeros((10,1)), -3.14),
        ('GridQubit', [3, 1], 3.14*np.ones((3,1)), 3.14*np.ones((3,0)), np.zeros((3,1)), -3.14),
        ('GridQubit', [11, 1], 3.14*np.ones((11,1)), 3.14*np.ones((11,0)), np.zeros((11,1)), -3.14),
        ('GridQubit', [1, 2], 3.14*np.ones((0,2)), 3.14*np.ones((1,1)), np.zeros((1,2)), -3.14),
        ('GridQubit', [1, 10], 3.14*np.ones((0,10)), 3.14*np.ones((1,10)), np.zeros((1,10)), -3.14),
        ('GridQubit', [1, 3], 3.14*np.ones((0,3)), 3.14*np.ones((1,3)), np.zeros((1,3)), -3.14),
        ('GridQubit', [1, 11], 3.14*np.ones((0,11)), 3.14*np.ones((1,11)), np.zeros((1,11)), -3.14),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ('GridQubit', [2, 2], np.ones((0,2))/2, np.ones((2,2))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((3,2))/2, np.ones((2,2))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,0))/2, np.ones((2,2))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,3))/2, np.ones((2,2))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((3,1))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((0,1))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((2,3))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((2,0))/5, np.ones((2,2))),
        ('GridQubit', [2, 2], np.ones((2,2))/2, np.ones((2,2))/5, np.ones((2,1))),
    ]
)
...
@pytest.mark.parametrize(
    'qubittype, n, j_v, j_h, h', 
    [
        ('GridQubit', [2, 2], np.ones((1,2))/2, np.ones((2,1))/5, np.ones((2,2))),
        ('GridQubit', [2, 1], np.ones((1,1))/2, np.ones((2,0))/5, np.reshape(np.array((0,-1)),(2, 1))),
        ('GridQubit', [3, 1], np.reshape(np.array((0,-1)),(2, 1)), np.ones((3,0)), np.ones((3,1))),
        ('GridQubit', [1, 3], np.ones((0,3)), np.reshape(np.array((0,-1)),(1, 2)), np.ones((1,3))),
    ]
)
