...
def test_create(x_dimension,y_dimension,tunneling,coulomb,hamiltonian_options):
    fermi_hubbard_hamiltonian = FermiHubbardModel(x_dimension=x_dimension,
...
def test_encode(transform_name):
    fermi_hubbard_hamiltonian = FermiHubbardModel(x_dimension=3,y_dimension=3,tunneling=1,coulomb=1,encoding_options={"encoding_name": transform_name})
    hamiltonian = fermi_hubbard_hamiltonian._encode_fock_hamiltonian()

def test_encode_wrong():
    fermi_hubbard_hamiltonian = FermiHubbardModel(x_dimension=3,y_dimension=3,tunneling=1,coulomb=1)
    with pytest.raises(KeyError):
        fermi_hubbard_hamiltonian.encoding_options["encoding_name"] = "wrong_encoding"
        hamiltonian = fermi_hubbard_hamiltonian._encode_fock_hamiltonian()

initial_state_data = [{"Nx":Nx,"Ny":Ny,"Nf":Nf} 
                        for Nx in range(2,3)
                        for Ny in range(1,3)
...
def initial_state(Nx,Ny,Nf):
    ...
    sparse_hamiltonian=of.get_sparse_operator(fermi_hubbard.non_interacting_model.fock_hamiltonian)
    ground_energy,ground_state = of.jw_get_ground_state_at_particle_number(sparse_hamiltonian, fermi_hubbard.Nf)
