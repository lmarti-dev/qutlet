...
def _first_order_trotter_circuit(   self, 
    ...
    #res.append(self.trotter._get_gate_from_paulistring2(self, pstr, hamiltonian, t))
    #Copy the Trotter layer *m times.
    #return res
    
    #return cirq.Circuit(*[self.trotter._get_gate_from_paulistring(self, pstr)**np.real(2/np.pi * float(t) * float(np.real(hamiltonian._linear_dict[pstr]))) for pstr in hamiltonian._linear_dict])
    return cirq.Circuit(*[self.trotter._get_gate_from_paulistring2(self, pstr, hamiltonian, t) for pstr in hamiltonian._linear_dict])
...
def _get_gate_from_paulistring2( self,
                                pauli_string: frozenset,
                                hamiltonian: cirq.PauliSum, 
                                t: Real):
    #Try to just return appropiate gates
