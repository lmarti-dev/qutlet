...
def LogSweepProtocol(self) -> None:
    ...
    if self.cooling.options["emin"] is None or self.cooling.options["emax"] is None:
        ...
        print(
            "Using default values for emin: {} and emax: {} and spectral spread: {}".format(
                e_min, e_max, spectral_spread
            )
        )
    else:
    ...
    for k in range(K):
        ...
        if self.cooling_type == "NA":
            ...
            # Trotter Layer
            c.append(
                self.trotter.get_trotter_circuit_from_hamiltonian(
                    self, self.hamiltonian, self.t, q, m
                )
            )
            # Reset Layer
            ...
            for n0 in range(self.m_sys.n[0]):
                for n1 in range(self.m_sys.n[1]):
                    ...
                    for pauli in [cirq.X, cirq.Y, cirq.Z]:
                        self._TwoQubitGates[
                            self.nbr_2Q_sys + self.nbr_2Q_anc : self.nbr_2Q
                        ] = [lambda q1, q2: pauli(q1) * cirq.X(q2)]
                        # --- and Hamiltonian
                        self._set_hamiltonian()
                        # Trotter Layer
                        c.append(
...
def __get_default_e_m(self) -> List[Real]:
    ...
    _N = 2**_n
    if (
        np.size(self.m_sys.eig_val) != _N
        or (np.shape(self.m_sys.eig_vec) != np.array((_N, _N))).all()
    ):
    ...
    e_max = max(
        self.cooling.orth_norm(
            1.0j
            * self.cooling.commutator(
                np.array(
                    pauli(self.m_sys.qubits[i][j]).matrix(
                        self.cooling.flatten(self.m_sys.qubits)
                    )
                ),
...
def __get_Log_Sweep_parameters(
    self, e_min: Real, e_max: Real, k: np.uint
) -> List[Real]:
    """
...
def BangBangProtocol(self) -> None:
    ...
    if self.cooling_type == "NA":
        for system in [cool_x, cool_y, cool_z]:
            # Trotter Layer
            c.append(
                system.trotter.get_trotter_circuit_from_hamiltonian(
                    system, system.hamiltonian, system.t, q, m
                )
            )
            # Reset Layer
            c.append(self.cooling._reset_layer(self))
...
def __get_Bang_Bang_parameters(self, pauli: cirq.Gate) -> List[Real]:
    ...
    tau: Real
        Simulation time
    """
    e = self.cooling.orth_norm(
        self.cooling.commutator(
            pauli(self.m_sys.qubits[0][0]).matrix(
                self.cooling.flatten(self.m_sys.qubits)
            ),
            self.m_sys.hamiltonian.matrix(),
        )
...
def __config_system(
    system: object, e: Real, gamma: Real, t: Real, pauli: cirq.Gate
) -> object:
    """
    ...
    system._TwoQubitGates[system.nbr_2Q_sys + system.nbr_2Q_anc : system.nbr_2Q] = [
        lambda q1, q2: pauli(q1) * cirq.X(q2)
    ]
    system._set_hamiltonian()
    return system
