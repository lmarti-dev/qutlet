...
def pauli_string_set(qubits:"list[cirq.Qid]",min_len:int,max_len:int,pauli_list:list=None,anti_hermitian:bool=True):
    pauli_set=[]
...
def fermionic_fock_set(shape:tuple,neighbour_order:int,excitation_order:int,periodic:bool=False, diagonal: bool=False,coeff:float=0.5,anti_hermitian:bool=True):
    ...
    for i in range(numcols*numrows):
            neighbours = utils.grid_neighbour_list(i,shape,neighbour_order,periodic=periodic,diagonal=diagonal)
            for term_order in range(2,2**(excitation_order)+1,2):
                combinations = itertools.combinations_with_replacement(neighbours[1:],term_order-1)
                for comb in combinations: 
                    fock_set.append(cqutils.even_excitation(coeff=coeff,indices=(i,)+comb,anti_hermitian=anti_hermitian))
    return fock_set
...
def fermionic_paulisum_set(model:FermionicModel,set_options:dict):
    ...
    for fop in fermionic_set:
        psum = model.encode_model(fermion_hamiltonian=fop,qubits=model.flattened_qubits,encoding_options=model.encoding_options)
        if psum != 0:
            paulisum_set.append(psum)
    return paulisum_set

def compute_noiseless_gradient(ham:np.ndarray,op:np.ndarray,wf:np.ndarray,verbose:bool=False) -> float:
...
def compute_full_gradient(ham:np.ndarray,op: np.ndarray,wf:np.ndarray,eps:float=1e-5,verbose:bool=False) -> float:
    ...
    wfexp = scipy.sparse.linalg.expm_multiply(A=op,B=wf,start=-eps,stop=eps,num=2,endpoint=True)
...
def filter_out_identity(psum):
    ...
    if isinstance(psum,cirq.PauliString):
        if cqutils.pauli_str_is_identity(pstr=psum):
            raise ValueError("Trying to filter out the remove the identity in a PauliString consisting only of the identity: {}".format(psum))
        else:
            return psum
...
def exp_from_pauli_sum(pauli_sum: cirq.PauliSum, theta):
    # PauliSumExponential adds a 1j so need to input a hermitian term
...
def get_best_gate(model:AbstractModel,paulisum_set:list,param_name:str,tol:float,wf=None,verbose:bool=False):
    if wf is None:
        circ = cqutils.populate_empty_qubits(model=model)
        wf = model.simulator.simulate(circ,param_resolver=cqutils.get_param_resolver(model=model,param_values=model.circuit_param_values)).final_state_vector
    ...
    for ps in paulisum_set:
        ...
        grad_values.append(compute_noiseless_gradient(
                            ham=ham,
                            op=op,
                            wf=wf,
                            verbose=False))
        # grad_values_full.append(compute_full_gradient(
        #                     ham=ham,
        #                     op=op,
        #                     wf=wf,
        #                     verbose=False))
    ...
    if tol is not None and grad_values[max_index] < tol:
        print_if_verbose("gradient ({grad:.2f}) is smaller than tol ({tol}), exiting".format(grad=grad_values[max_index],tol=tol),verbose=verbose)
        # iteration process is done, gradient < tol, or if stopping is set with max depth, continue
        return None
    else:
        print_if_verbose("gradient ({grad}) is larger than tol ({tol}), continuing".format(grad=grad_values[max_index],tol=tol),verbose=verbose)
...
def circuit_iterating_step(model:AbstractModel,paulisum_set:list,tol:float=1e-15,wf=None,verbose:bool=False):
    res = get_best_gate(model=model,paulisum_set=paulisum_set,param_name=utils.random_word(lenw=4,Nwords=1),tol=tol,wf=wf,verbose=verbose)
    if res is None:
        ...
        cqutils.match_param_values_to_symbols(model=model,symbols=model.circuit_param,default_value="random")
        print_if_verbose("best gate found and added: {best_gate}".format(best_gate=exp_gates),verbose=verbose)
        #print_if_verbose(model.circuit,verbose=verbose)
        return False


# pick random gate for benchmarking
def random_iterating_step(model:AbstractModel,paulisum_set:list,verbose:bool=False,default_value="random"):
    ...
    for gate in exp_gates:
        model.circuit += gate
    model.circuit_param.append(theta)
    cqutils.match_param_values_to_symbols(model=model,symbols=model.circuit_param,default_value=default_value)
