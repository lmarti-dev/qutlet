...
class Adiabatic(SpinModelFC):
    """
        ...
        _get_minimal_energy_gap(self, times: np.ndarray = None) -> float
            Returns
            ---------
            float
                Minimum Energy gap along sweep
        
        _get_groundstate_at_time(self, time: float) -> np.array
            Returns
            ---------
            np.array
    ...
    def _set_initial_state_for_sweep(self) -> None:
        """
        Sets initial state for sweep which is ground state of H0 by calling _get_groundstate_at_time:
    ...
    def _set_output_state_for_sweep(self) -> None:
        """
        Sets targeted output state of sweep which is ground state of H1 by calling _get_groundstate_at_time:
