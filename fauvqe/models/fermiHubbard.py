...
class FermiHubbardModel(FermionicModel):
    ...
    def set_initial_state_circuit(
        self, name: str, Nf: int, initial_state: Union[int, Sequence[int]] = None
    ):
    ...
    def common_Z_snakes(name, dimx, dimy):
        ...
        if name == "rowmajor":
            ...
            Z_snake_up = utils.flip_cross_rows(
                np.reshape(np.arange(dimy * dimx), (dimx, dimy)),
                flip_odd=bool(dimx % 2),
            )
            Z_snake_down = utils.flip_cross_rows(
                np.reshape(np.arange(dimy * dimx, 2 * dimy * dimx), (dimx, dimy)),
                flip_odd=1,
            )
