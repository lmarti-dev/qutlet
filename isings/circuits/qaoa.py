...
def set_symbols(self, p):
  ...
  assert(isinstance(p, (int, np.int_))), \
...
def _set_beta_values(self, beta_values):
  try:
    assert(np.size(beta_values) == self.p), \
    "Error: size(beta_values !=  p; {} != {}".format(np.size(beta_values), self.p)
  except AttributeError:
    #set p to length beta_values if it does not exist
    print('Set self.p to np.size(beta_values) = {}, as not defined'\
          .format(np.size(beta_values)))
...
def _set_gamma_values(self, gamma_values):
  try:
    assert(np.size(gamma_values) == self.p), \
    "Error: size(gamma values) !=  p; {} != {}".format(np.size(gamma_values), self.p)
  except AttributeError:
    #set p to length beta_values if it does not exist
    print('Set self.p to np.size(gamma_values) = {}, as not defined'\
          .format(np.size(gamma_values)))
